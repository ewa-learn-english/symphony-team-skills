...
Return exactly one disposition in the thread's final response: `ready`, or `not ready` with blocking findings, optional non-blocking findings, and clearing conditions. Include any created commit. Report `ready` when the final assigned phase diff and phase-owned proof are ready, and list downstream phases separately.
